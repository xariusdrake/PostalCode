import * as common from "@nestjs/common";
import * as swagger from "@nestjs/swagger";
import * as nestMorgan from "nest-morgan";
import * as nestAccessControl from "nest-access-control";
import * as basicAuthGuard from "../auth/basicAuth.guard";
import * as abacUtil from "../auth/abac.util";
import { isRecordNotFoundError } from "../prisma.util";
import * as errors from "../errors";
import { StoreCodeService } from "./storeCode.service";
import { StoreCodeCreateInput } from "./StoreCodeCreateInput";
import { StoreCodeWhereInput } from "./StoreCodeWhereInput";
import { StoreCodeWhereUniqueInput } from "./StoreCodeWhereUniqueInput";
import { StoreCodeUpdateInput } from "./StoreCodeUpdateInput";
import { StoreCode } from "./StoreCode";

@swagger.ApiBasicAuth()
@swagger.ApiTags("store-codes")
@common.Controller("store-codes")
export class StoreCodeController {
  constructor(
    private readonly service: StoreCodeService,
    @nestAccessControl.InjectRolesBuilder()
    private readonly rolesBuilder: nestAccessControl.RolesBuilder
  ) {}

  @common.UseInterceptors(nestMorgan.MorganInterceptor("combined"))
  @common.UseGuards(basicAuthGuard.BasicAuthGuard, nestAccessControl.ACGuard)
  @common.Post()
  @nestAccessControl.UseRoles({
    resource: "StoreCode",
    action: "create",
    possession: "any",
  })
  @swagger.ApiCreatedResponse({ type: StoreCode })
  @swagger.ApiForbiddenResponse({ type: errors.ForbiddenException })
  async create(
    @common.Query() query: {},
    @common.Body() data: StoreCodeCreateInput,
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<StoreCode> {
    const permission = this.rolesBuilder.permission({
      role: userRoles,
      action: "create",
      possession: "any",
      resource: "StoreCode",
    });
    const invalidAttributes = abacUtil.getInvalidAttributes(permission, data);
    if (invalidAttributes.length) {
      const properties = invalidAttributes
        .map((attribute: string) => JSON.stringify(attribute))
        .join(", ");
      const roles = userRoles
        .map((role: string) => JSON.stringify(role))
        .join(",");
      throw new errors.ForbiddenException(
        `providing the properties: ${properties} on ${"StoreCode"} creation is forbidden for roles: ${roles}`
      );
    }
    // @ts-ignore
    return await this.service.create({
      ...query,
      data: data,
      select: {
        qrCode: true,
        createdAt: true,
        id: true,
        isDelete: true,
        storeId: true,
        updatedAt: true,
      },
    });
  }

  @common.UseInterceptors(nestMorgan.MorganInterceptor("combined"))
  @common.UseGuards(basicAuthGuard.BasicAuthGuard, nestAccessControl.ACGuard)
  @common.Get()
  @nestAccessControl.UseRoles({
    resource: "StoreCode",
    action: "read",
    possession: "any",
  })
  @swagger.ApiOkResponse({ type: [StoreCode] })
  @swagger.ApiForbiddenResponse()
  async findMany(
    @common.Query() query: StoreCodeWhereInput,
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<StoreCode[]> {
    const permission = this.rolesBuilder.permission({
      role: userRoles,
      action: "read",
      possession: "any",
      resource: "StoreCode",
    });
    const results = await this.service.findMany({
      where: query,
      select: {
        qrCode: true,
        createdAt: true,
        id: true,
        isDelete: true,
        storeId: true,
        updatedAt: true,
      },
    });
    return results.map((result) => permission.filter(result));
  }

  @common.UseInterceptors(nestMorgan.MorganInterceptor("combined"))
  @common.UseGuards(basicAuthGuard.BasicAuthGuard, nestAccessControl.ACGuard)
  @common.Get("/:id")
  @nestAccessControl.UseRoles({
    resource: "StoreCode",
    action: "read",
    possession: "own",
  })
  @swagger.ApiOkResponse({ type: StoreCode })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @swagger.ApiForbiddenResponse({ type: errors.ForbiddenException })
  async findOne(
    @common.Query() query: {},
    @common.Param() params: StoreCodeWhereUniqueInput,
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<StoreCode | null> {
    const permission = this.rolesBuilder.permission({
      role: userRoles,
      action: "read",
      possession: "own",
      resource: "StoreCode",
    });
    const result = await this.service.findOne({
      ...query,
      where: params,
      select: {
        qrCode: true,
        createdAt: true,
        id: true,
        isDelete: true,
        storeId: true,
        updatedAt: true,
      },
    });
    if (result === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return permission.filter(result);
  }

  @common.UseInterceptors(nestMorgan.MorganInterceptor("combined"))
  @common.UseGuards(basicAuthGuard.BasicAuthGuard, nestAccessControl.ACGuard)
  @common.Patch("/:id")
  @nestAccessControl.UseRoles({
    resource: "StoreCode",
    action: "update",
    possession: "any",
  })
  @swagger.ApiOkResponse({ type: StoreCode })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @swagger.ApiForbiddenResponse({ type: errors.ForbiddenException })
  async update(
    @common.Query() query: {},
    @common.Param() params: StoreCodeWhereUniqueInput,
    @common.Body()
    data: StoreCodeUpdateInput,
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<StoreCode | null> {
    const permission = this.rolesBuilder.permission({
      role: userRoles,
      action: "update",
      possession: "any",
      resource: "StoreCode",
    });
    const invalidAttributes = abacUtil.getInvalidAttributes(permission, data);
    if (invalidAttributes.length) {
      const properties = invalidAttributes
        .map((attribute: string) => JSON.stringify(attribute))
        .join(", ");
      const roles = userRoles
        .map((role: string) => JSON.stringify(role))
        .join(",");
      throw new errors.ForbiddenException(
        `providing the properties: ${properties} on ${"StoreCode"} update is forbidden for roles: ${roles}`
      );
    }
    try {
      // @ts-ignore
      return await this.service.update({
        ...query,
        where: params,
        data: data,
        select: {
          qrCode: true,
          createdAt: true,
          id: true,
          isDelete: true,
          storeId: true,
          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.UseInterceptors(nestMorgan.MorganInterceptor("combined"))
  @common.UseGuards(basicAuthGuard.BasicAuthGuard, nestAccessControl.ACGuard)
  @common.Delete("/:id")
  @nestAccessControl.UseRoles({
    resource: "StoreCode",
    action: "delete",
    possession: "any",
  })
  @swagger.ApiOkResponse({ type: StoreCode })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  @swagger.ApiForbiddenResponse({ type: errors.ForbiddenException })
  async delete(
    @common.Query() query: {},
    @common.Param() params: StoreCodeWhereUniqueInput
  ): Promise<StoreCode | null> {
    try {
      return await this.service.delete({
        ...query,
        where: params,
        select: {
          qrCode: true,
          createdAt: true,
          id: true,
          isDelete: true,
          storeId: true,
          updatedAt: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }
}
